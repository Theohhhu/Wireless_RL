import numpy as np
import torch
from myenv.hvenv_32step_classical_init_nstep_niteration   import hvenv
from rlnnet.rl_bn_dqn  import DQN
...
SAVE_PATH = '../sota/weight/train_32step_syc3200_classical_init_bn_8step_1iteration_multi_ddqn.pth'
...
def main():
    ...
    while True:
        ...
        if i%100==0:
            obs_val,reward_val,reward_record = eval(rlnnet,envs_val,obs_val,epsilon,cuda0,reward_val,reward_record)

def eval(rlnnet,envs,obs,epsilon,cuda0,reward,reward_record):
    rlnnet.eval()
    print('-------eval testing-------')
    while True:
        _, obs, done, reward = play_step(rlnnet, envs, obs, epsilon, cuda0, reward)
        if done:
            if  np.mean(reward)>reward_record:
                reward_record = np.mean(reward)
                torch.save(rlnnet,
                           SAVE_PATH)
                print('save new weight')
            print('mean reward: '+ str(np.mean(reward))+' max reward: '+ str(reward_record))

            obs = envs.reset()
            reward = envs.reward()
            break
    print('-------eval end-------')
    return obs,reward,reward_record

def getConvMat(states):
    obs_v = states[:, :int(states.shape[1] / 2)]
    obs_h = states[:, -int(states.shape[1] / 2):]
    obs_v = obs_v.reshape((obs_v.shape[0], obs_v.shape[1], 1))
    obs_h = obs_h.reshape((obs_h.shape[0], 1, obs_h.shape[1]))
    obs_total = np.matmul(obs_v, obs_h)

    obs_real = np.real(obs_total)
    obs_imag = np.imag(obs_total)
    return  np.stack((obs_real, obs_imag), axis=1).reshape(states.shape[0], -1)
...
def calc_loss(batch, net, tgt_net, device):
    ...
    obs_input = getConvMat(states)
    obs_input_next = getConvMat(next_states)
    ...
    # obs_next_v = states[:, :int(states.shape[1] / 2)]
    # obs_next_h = states[:, -int(states.shape[1] / 2):]
    # obs_next_v = obs_next_v.reshape((obs_next_v.shape[0], obs_next_v.shape[1], 1))
    # obs_next_h = obs_next_h.reshape((obs_next_h.shape[0], 1, obs_next_h.shape[1]))
    # obs_next_total = np.matmul(obs_next_v, obs_next_h)
...
def action_choose(nnet,envs,obs,epsilon,device):
    # 分解obs
    obs_v = obs[:,:int(obs.shape[1]/2)]
    obs_h = obs[:,-int(obs.shape[1]/2):]
    obs_v = obs_v.reshape((obs_v.shape[0],obs_v.shape[1],1))
    obs_h = obs_h.reshape((obs_h.shape[0],1,obs_h.shape[1]))
    obs_total = np.matmul(obs_v,obs_h)
    ...
    obs_real = np.real(obs_total)
    obs_imag = np.imag(obs_total)
    obs_input = np.stack((obs_real, obs_imag), axis=1).reshape(obs.shape[0],-1)
    #贪婪策略决定动作
